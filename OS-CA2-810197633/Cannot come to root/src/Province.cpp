...
{
    mkfifo(PROVINCE_FILE_PATH, 0666);
}
...
void Province::forkCity()
{
    for (int i = 0; i < cityPipes.size(); i++)
    {
        ...
        else
        {
            ...
            wait(NULL);
            int fd = open(CITY_FILE_PATH, O_RDONLY);
            char cost[100];
            read(fd, cost, 100);
...
{
    char message[2000];
    strcpy(message, (directory + SLASH + cityNames[cityNumber] + LESS + command).c_str());
...
void Province::sendGoal()
{
    ...
    //     cout << costs[i] << endl;
    int fd = open(PROVINCE_FILE_PATH, O_WRONLY);
    if (fd == -1)
    {
